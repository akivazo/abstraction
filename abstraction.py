...
class abstraction:
    ...
    def __create_node(self, event: event):
        new_node = node(intensity=self.__get_event_intentsity(event), event=event)
    ...
    def __find_branch(self, node: node):
        ...
        if self.__is_part_of_branch(node):
            ...
            if rest_of_branch_node is not None:
                ...
                self.__propigate_to_next_layer(branch_node.get_event())
        return branch_node
    ...
    def process(self, event: event): 
        ...
        if last_node is not None:
            last_node.connect_to_node(node=new_node, strengrh=self.__get_node_intensity(new_node))

    def learn(self):
        while True:
            new_event = self.__event_supplier()
            if isinstance(new_event, eof_event) or new_event is None:
                self.__summarize_isolated_branches()
                break
